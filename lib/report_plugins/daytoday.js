...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  var potassiumAverage = potassiumSum / datastorage.alldays;
  ...
  if (options.insulindistribution) {
    ...
    html += '<b>' + translate('Potassium average') + ':</b> ' + potassiumAverage.toFixed(0) + 'g';
  ...
  function drawChart (day, data, options) {
    ...
    data.treatments.forEach(function(treatment) {
      ...
      if (treatment.carbs && options.carbs) {
        ...
        if (treatment.potassium) label += ' / ' + treatment.potassium + ' g';
    ...
    if (options.insulindistribution) {
      ...
      $('<tr><td>' + translate('Total potassium') + ':</td><td align="right">' + data.dailyPotassium + ' g</td></tr>').appendTo(table);
    ...
    potassiumSum += data.dailyPotassium;
